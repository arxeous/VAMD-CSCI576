...
typedef struct VideoState {
	...
	int                 audioPacketSize;
	double				audioDiffCum;		// Used in AV difference average computation
...
{
	double delta;
	delta = (av_gettime() - state->videoCurrPtsTime) / 1000000.0;
	...
	if (state->videoStream) {
		if (state->pictQSize == 0) {
			...
			if (state->avSyncType != AV_SYNC_VIDEO_MASTER)
			{
				...
				ref_clock = get_audio_clock(state);
			...
			actual_delay = state->frameTimer - (av_gettime() / 1000000.0);
...
//		if (state->pictQSize == 0)
//		{
//			schedule_refresh(state, 10);
//		}
//		else
//		{
//			vw = &state->pictQ[state->pictQRIndex];
//
//			schedule_refresh(state, 80);
//			video_display(state);
//
//			if (++state->pictQRIndex == VIDEO_PICTURE_QUEUE_SIZE)
//			{
...
//			}
//			SDL_LockMutex(state->pictQMutex);
//			printf("This should decrement our pictqsize and signal our condition.\n");
//			state->pictQSize--;
//			SDL_CondSignal(state->pictQCond);
//			SDL_UnlockMutex(state->pictQMutex);
//		}
